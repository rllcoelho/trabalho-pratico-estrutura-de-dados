...
void menuConta(NoArvore** arvore){
    int opcao, cpf, confirma, numConta, posicao;
    char aux[30];
    printf("Digite o CPF do cliente: ");
    fgets(aux, sizeof(aux), stdin);
    sscanf(aux, "%d", &cpf);
    ...
    switch(opcao){
        ...
        case 2:
            ...
            int numContaAntigo = numConta;
            printf("Digite um novo número para a conta: ");
            fgets(aux, sizeof(aux), stdin);
            sscanf(aux, "%d", &numConta);
            conta->numConta = numConta;
            printf("\n");
            printf("A conta de número %d agora possui o número %d \n", numContaAntigo, numConta);
            break;
        case 3:
            printf("Digite o número da conta a ser exibida: ");
            fgets(aux, sizeof(aux), stdin);
            sscanf(aux, "%d", &numConta);
            conta = exibeConta(contas, numConta, 2);
            if (!conta){
        ...
        case 4:
            ...
            printf("\nTem certeza de que deseja remover desse cliente a conta %d? (1 para sim, 2 para não)\n", conta->numConta);
            fgets(aux, sizeof(aux), stdin);
            sscanf(aux, "%d", &confirma);
            if (confirma == 1) {
                posicao = posicaoNoListaDupla(contas, numConta);
                removeNoListaDupla(&contas, posicao);
            }
            break;
        case 5:
